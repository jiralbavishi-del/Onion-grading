...
import { DEFECT_SAMPLE_IMAGES } from '../onionImagesData';
...
export function ImageDropzone({
  ...
  const getSampleTag = (tag) => {
    ...
    return key ? `[${t[key] || tag.replace(/[\[\]]/g, '')}]` : tag;
  };
    ...
    <div id="section-photos" className="bento-tile p-5 sm:p-6 space-y-3.5 flex flex-col h-full">
      ...
      <div className="space-y-2 relative">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
          {DEFECT_SAMPLE_IMAGES.map((sample) => {
            ...
            return (
              ...
              >
                <div className="relative rounded-2xl overflow-hidden border border-stone-200/90 bg-stone-900 h-32 shadow-xs group-hover:shadow-md transition-all duration-300">
                  ...
                  <img
                    ...
                    loading="lazy"
                  />
                  {/* Subtle Dark Gradient Overlay for Tag Readability */}
                  <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/25 to-transparent" />

                  {/* Title & Overlay Tags */}
                  <div className="absolute inset-x-0 bottom-0 p-2.5 space-y-1 z-10">
                    <span className="text-[10px] font-bold text-white line-clamp-1 drop-shadow-xs block">
                      {getSampleTitle(sample.title)}
                    </span>
                    <div className="flex flex-wrap gap-1">
                      <span className={`text-[8px] font-mono font-bold px-1.5 py-0.5 rounded border backdrop-blur-md ${sample.tagBg}`}>
                        {getSampleTag(sample.tag)}
                      </span>
                      <span className={`text-[8px] font-mono font-bold px-1.5 py-0.5 rounded border backdrop-blur-md ${sample.badgeBg}`}>
